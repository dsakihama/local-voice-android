...
- [x] **Faithfulness re-test passed (2026-06-23)** — E2B-CPU cleans Text fillers without fabrication AND keeps Notes as prose (no stray checkboxes). The original Gemma 3 1B failure mode is fixed. **E2B is the confirmed model for now.** E4B `.litertlm` kept on device for A/B as we iterate on agent files (swap = one-line `MODEL_FILENAME` + rebuild).
