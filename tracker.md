...
**Where things stand (2026-07-10):** Last commit is still `3ab3c80` (Jun 29) — no cleanup/polish work has landed since. `docs/model-and-pipeline-review-2026-07-05.md` (11 findings, F1–F11) and `docs/claude-code-prompt-pack-2026-07-05.md` (7 ordered Claude Code prompts covering them) define the requirements for the **polishing stage** (LLM cleanup between STT and share/injection). Re-verified against the current codebase today — zero drift, all 7 prompts still accurate and ready to run in order (1 → 2 → 3 → 4 → 5 → 6 → 7; 2 and 7 can slot anywhere). One path correction applied (Prompt 1: module build file is `android/app/build.gradle.kts`, not `app/build.gradle.kts`, after the Gradle root got flattened to the project root). Prompt 5 (agent files / few-shot) is still the ⭐ roadmap item but is gated behind Prompts 2–4 (bugfix, engine lifecycle, token budget) per the pack's ordering.

**Next up (in priority order):**
1. **⭐ Agent files / few-shot wiring (the coachability payoff)** — feed examples into `LlmCleanup.clean(intent, rawText, examples)`. The plumbing already exists: `CleanupPrompts.buildParts()` injects an `EXAMPLES OF YOUR PREVIOUS CLEANUPS:` block into the `systemInstruction`; `TranscribeViewModel.runCleanup` currently passes nothing (`emptyList()`). This is the "less free-form, more steering by example" goal that motivated the Gemma 4 move. **Open design questions:** (a) source of examples — curated per-intent "agent files" (static, hand-authored) vs. captured Room `training_pairs` (user edits cleaned output → store raw→final); (b) selection — all examples for the intent vs. retrieve most-relevant few (cap ~10 per `buildExampleBlock`). Now unblocked; A/B harness (E2B/E4B runtime toggle) is in place to evaluate prompt/example changes without rebuild.
...
**Specific models to download — get the prebuilt `.litertlm` from Hugging Face, NOT Kaggle:**
- ⚠️ **Kaggle correction:** the Kaggle `google/gemma-4` **"Other"** tab does **not** hold ready-to-run `.litertlm`. It holds **QAT source checkpoints in compressed-tensors ("ct") format**: `...-qat-w4a16-ct` = vLLM/cloud (ignore), `...-qat-mobile-ct` = mobile-targeted QAT *checkpoint* that still needs packaging into `.litertlm` (conversion path still being clarified upstream — google-ai-edge/LiteRT-LM #2497). Only use these if doing your own quantize→package step.
- ✅ **Drop-in files live on HF `litert-community`** (community-built LiteRT-LM packages). This is the download source.
...
- [x] **Gemma 4 E4B swap (code)** — `MediaPipeLlmCleanup` loads `gemma-4-E4B-it.litertlm`; `runCleanup` routes all intents through the LLM with programmatic fallback. Pushed `gemma-4-E4B-it.litertlm` (3,659,530,240 bytes, verified) to device. **WIP checkpoint committed — loads but not usable yet (see findings).**
  ...
  **🔴 On-device findings (2026-06-23, Pixel 10 Pro XL):**
  ...
  - **Conclusion:** MediaPipe is maintenance-mode with no working Gemma 4 path here → migrated to LiteRT-LM. (Hoped this would unlock GPU/NPU; it did **not** on this device — see outcome below.)
- [x] **Migrated runtime MediaPipe `tasks-genai` → LiteRT-LM** (`com.google.ai.edge.litertlm:litertlm-android:0.13.1`) — `MediaPipeLlmCleanup.kt` → `LlmCleanup.kt`: persistent `Engine` + fresh `Conversation` per call, `systemInstruction` from `CleanupPrompts.buildParts()`, greedy sampler (topK=1/temp=0), `maxNumTokens=2048`. Public surface unchanged; ViewModel only renamed refs. Compiles + runs. See `design/litert-lm-migration.md`.

  **🟢 Landed config (2026-06-23):** **Gemma 4 E2B generic `.litertlm` on `Backend.CPU()`** — acceptable latency + faithful (Text filler cleanup confirmed, no 1B-style fabrication). **GPU is unusable on this device:** `Backend.GPU()` fails engine creation for *both* E2B and E4B (`LiteRtLmJniException` at `llm_litert_compiled_model_executor.cc:1951`); the bundled OpenCL accelerator can't compile Gemma 4 on the Tensor G5. The `_Google_Tensor_G5` AOT build is **not runnable** — needs a Google Tensor dispatch lib absent from the public AAR; `Backend.NPU()` is Qualcomm-only. **E4B-CPU** = faithful-but-slower alternative (one-line `MODEL_FILENAME` swap).
- [x] **Faithfulness re-test passed (2026-06-23)** — E2B-CPU cleans Text fillers without fabrication AND keeps Notes as prose (no stray checkboxes). The original Gemma 3 1B failure mode is fixed. **E2B is the confirmed model for now.**
- [x] **Runtime model toggle** — `CleanupModel` enum (E2B/E4B), persisted in DataStore (`AppSettings.cleanupModel`), switchable from the **Settings gear dropdown** with live engine reload (`LlmCleanup.useModel()`). Both `.litertlm` files live on device; A/B E2B↔E4B with no rebuild.
- [ ] **Watch for a working GPU/NPU path** — newer `litertlm-android` AAR with a Google Tensor dispatch lib, or an OpenCL fix for Gemma 4. Would unlock acceleration (E2B currently CPU-only).
